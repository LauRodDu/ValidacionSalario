...
def calcular_salario(salario_base, horas_extras, bonificaciones):

    valor_hora_normal = salario_base / 192
    valor_hora_extra = valor_hora_normal * 1.25
    valor_bonificacion = salario_base * 0.05
    valor_total_horas_extra = horas_extras * valor_hora_extra
    total_bonificacion = bonificaciones * valor_bonificacion

    salario_total = salario_base + valor_total_horas_extra + total_bonificacion

    descuento_salud = salario_total * 0.035
    descuento_pension = salario_total * 0.04
    descuento_caja_compensacion = salario_total * 0.01

    salario_descuentos = salario_total - (descuento_salud + descuento_pension + descuento_caja_compensacion)


    return salario_descuentos


# 
datos = input().split()

salario_base = float(datos[0])
horas_extras = float(datos[1])
bonificaciones = float(datos[2])

#salario_base = int(input())
#horas_extras = int(input())
#bonificaciones = int(input())
# Cálculo del salario
salario_final = calcular_salario(salario_base, horas_extras, bonificaciones)
# Impresión del resultado
# print(f"{salario_final:.1f}")
